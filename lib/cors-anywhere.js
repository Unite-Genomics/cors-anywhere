...
if (providerFileExists) {
  ...
  const filter = `
    [.core_provider[] | select(.use_proxy == true) | .FHIR_api_base_uri]
  `;
  jq.run(filter, process.env.PROVIDER_JSON_FILE, {}).then((result) => {
    JSON.parse(result)?.forEach((item) => {
      whitelistEndpoints[item] = true;
    });
  }).catch((err) => {
    console.error(err);
  });
  ...
  client.connect().then(() => {
    ...
    client.query(query).then((result) => {
      result.rows?.forEach(row => {
        whitelistEndpoints[row['FHIR_api_base_uri']] = true;
      });
    }).catch((err) => {
      console.error(err);
    });
...
const isProviderAllowed = (provideEndpoint) => {
  return !!whitelistEndpoints[provideEndpoint];
};

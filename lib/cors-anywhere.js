...
// © 2024 - Damjan Ravnjak <damjan@unitegenomics.com>
...
const fs = require('fs');
const providerFileExists = fs.existsSync(process.env.PROVIDER_JSON_FILE);

let client = null;

if (providerFileExists) {
  console.log(`Reading providers from ${process.env.PROVIDER_JSON_FILE} file.`);
} else {
  const pg = require('pg');
...
const isProviderAllowed = async (provideEndpoint) => {
  if (client) {
    const query = {
      name: 'fetch-providers',
      text: 'SELECT * FROM core_provider where use_proxy = true and "FHIR_api_base_uri" = $1::text',
      values: [provideEndpoint],
    };
  ...
  if (providerFileExists) {
    const filter = `
      .core_provider[] | select(.FHIR_api_base_uri == "${provideEndpoint}" and .use_proxy == true)
    `;
    const result = await jq.run(filter, process.env.PROVIDER_JSON_FILE, {});
    return !!result;
  }
};
...
function showUsage(help_file, headers, response) {
  if (process.env.DEPLOYMENT_TYPE !== 'dev') {
...
function getHandler(options, proxy) {
  ...
  return async function (req, res) {
    ...
    const isAllowedOrigin = await isProviderAllowed(origin);
    if (!isAllowedOrigin) {
      res.writeHead(400, 'Invalid origin', cors_headers);
      res.end('Invalid origin: ' + origin);
      return;
    }

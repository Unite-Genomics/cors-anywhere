...
if (providerJSONFileExists) {
  ...
  try {
    const fileLoaded = JSON.parse(fs.readFileSync(process.env.PROVIDER_JSON_FILE, 'utf8'));
...
const isProviderAllowed = (providerEndpoint) => {
  providerEndpoint = providerEndpoint.replace('.well-known/smart-configuration', '');
  providerEndpoint = providerEndpoint.replace('metadata', '');
  return !!whitelistEndpoints[providerEndpoint];
};
...
function getHandler(options, proxy) {
  ...
  return async function (req, res) {
    ...
    const isAllowedOrigin = await isProviderAllowed(location.href);
    if (!isAllowedOrigin) {
      res.writeHead(400, 'Invalid location', cors_headers);
      res.end('Invalid location: ' + location.href);
      return;
    }
